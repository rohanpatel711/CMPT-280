...
public class IterableTwoThreeTree280<K extends Comparable<? super K>, I extends Keyed280<K>> extends TwoThreeTree280<K, I> implements KeyedDict280<K,I> {
	...
	protected Pair280<TwoThreeNode280<K,I>, K> insert(TwoThreeNode280<K,I> root,
		...
		if( !root.isInternal() ) {
			...
			LinkedLeafTwoThreeNode280<K,I> oN = oldLeaf.next();
			oldLeaf.setNext(newLeaf);
			newLeaf.setPrev(oldLeaf);

			if (oN != null) {
				newLeaf.setNext(oN);	// set new Leaf's next node to be the oldNext
				oN.setPrev(newLeaf);	// set the old next's previous node to be the  new leaf
			}
	...
	protected void delete(TwoThreeNode280<K, I> root, K keyToDelete ) {
		if( root.getLeftSubtree().isInternal() ) {
			...
			if( root.getLeftSubtree().getKey1().compareTo(keyToDelete) == 0 ) {
				...
				LinkedLeafTwoThreeNode280<K,I> l = (LinkedLeafTwoThreeNode280<K,I>) root.getLeftSubtree();
				if (l.prev() == null) {
					if (this.cursor.getData().key().compareTo(keyToDelete) == 0) {
						this.goBefore();
					}
					this.smallest = l.next();
					l.next().setPrev(null);
				}
				else {
					if (this.cursor.getData().key().compareTo(keyToDelete) == 0) {
				...
				LinkedLeafTwoThreeNode280<K,I> m = (LinkedLeafTwoThreeNode280<K,I>) root.getMiddleSubtree();
				...
				LinkedLeafTwoThreeNode280<K,I> r = (LinkedLeafTwoThreeNode280<K,I>) root.getRightSubtree();
				if (this.cursor.getData().key().compareTo(keyToDelete) == 0) {
					this.cursor = r.prev();
				}
				if (r.next() == null) {
	...
	public K itemKey() throws NoCurrentItem280Exception {
		if (this.before() || this.after() || !this.itemExists()) {
			throw new NoCurrentItem280Exception("No current item exist from which we could obtain the key.");
		}
	...
	public void search(K k) {
		LinkedLeafTwoThreeNode280<K, I> foundItem =(LinkedLeafTwoThreeNode280<K, I>) find(this.rootNode, k);
		if (foundItem == null) {
			this.goAfter();
		}
		else {
			this.cursor = foundItem;
		}
	}
	...
	public void setItem(I x) throws NoCurrentItem280Exception, InvalidArgument280Exception {
		if (this.before() || this.after() || !this.itemExists()) {
			throw new NoCurrentItem280Exception("Cursor is currently not pointing at an item.");
		}
		else if (this.cursor.data.key() != x.key()) {
			throw new InvalidArgument280Exception("Item x does not have the same key as the current item in current cursor position");
		}
		this.cursor.data = x;
	}
	...
	public static void main(String args[]) {
		...
		System.out.println("---------------REGRESSION TEST BEGINS HERE-----------------");

// An example of instantiating an item. (you can remove this if you wish)
		Loot sampleItem = new Loot("King's Spear", 100);	
		Loot sampleItem1 = new Loot("Ultimate Sword", 1000);	
		Loot sampleItem2 = new Loot("Flame Arrow", 400);			
		Loot sampleItem3 = new Loot("Tribal Blade", 800);		
		Loot sampleItem4 = new Loot("Immortality Elixir", 300);				
		Loot sampleItem5 = new Loot("Beautiful Death", 1100);	
		Loot sampleItem6 = new Loot("Prince's Dagger", 600);		
		Loot sampleItem7 = new Loot("Unbreakable Shield", 900);		
		Loot sampleItem8 = new Loot("Phoenix Tears", 700);		
		Loot sampleItem9 = new Loot("Durable Spear", 500);		
		Loot sampleItem10 = new Loot("Invisibility Cloak", 200);		


		T.insert(sampleItem);	// Insert the first item

		T.goForth();	// move cursor to the next item
		if (T.item().key().compareTo("King's Spear") != 0) {
			System.out.println("Current cursor position should now be positioned at King's Spear");
		}
		...
		try {
			if (T.item().key().compareTo("King's Spear") == 0) {
				System.out.println("King's Spear should now be deleted and the item should now be null since tree is empty.");
			}
		...
		// Insert a series of items into our Iterable two three tree, including reinsertion of King's Spear
		T.insert(sampleItem);
		...
		T.search("Potato gun");
		if (!T.after()) {
			System.out.println("Cursor should point to the after position as the item having key Potato gun doesn't exist.");
		}

		T.search("Immortality Elixir");
		if (T.item().key().compareTo("Immortality Elixir") != 0) {
			System.out.println("Cursor should point at item having key as **Immortality Elixir**.");
		}
		T.goForth();
		if (T.item().key().compareTo("Invisibility Cloak") != 0 ) {
			System.out.println("Cursor should be point at item having key as **Invisibility Cloak**.");
		}
		T.deleteItem();
		if (T.itemKey().compareTo("King's Spear") != 0) {
			System.out.println("**Invisibility Cloak**, should now be deleted and Cursor should point to the next item i.e King's Spear.");
		}
		T.goForth();

		if (T.itemKey().compareTo("Phoenix Tears") != 0) {
			System.out.println("The key value of Item after **King's Spear** should be the **Phoenix Tears**.");
		}
		if (T.keyItemPair().secondItem().key() != "Phoenix Tears") {
			System.out.println("Cursor should point at the key of item pair having value as **Phoenix Tears**.");
		}
		...
		if (T.prev.getData().key().compareTo("Ultimate Sword") != 0) {
			System.out.println("The key of the last item should be **Ultimate Sword**.");
		}
		if (T.largest.getData().key().compareTo("Ultimate Sword") != 0) {
			System.out.println("The key of largest item should be **Ultimate Sword**.");
		}
		...
		if (T.item().key().compareTo("Durable Spear") != 0) {
			System.out.println("**Beautiful Death** is deleted, and the key of the first item should be **Durable Spear**.");
		}
		if (T.smallest.getData().key().compareTo("Durable Spear") != 0) {
			System.out.println("The key of the smallest item should be **Durable Spear**.");
		}

		T.deleteItem();

		if (T.item().key().compareTo("Flame Arrow") != 0) {
			System.out.println("The key of the first item should be **Flame Arrow**.");
		}
		if (T.smallest.getData().key().compareTo("Flame Arrow") != 0) {
			System.out.println("The key of the smallest item should be **Flame Arrow**.");
		}
		...
		T.search("Phoenix Tears");
		T.deleteItem();
		T.search("Phoenix Tears");
		if (!T.after()) {
			System.out.println("Phoenix Tears is deleted and doesn't exist anymore. Therefore, cursor should be on after position");
		}

		T.searchCeilingOf("Immortality Elixir");
		if (T.itemKey().compareTo("Immortality Elixir") != 0) {
			System.out.println("Cursor should be pointed at Immortality Elixir.");
		}
		T.goFirst();
		...
		Loot sampleItem11 = new Loot("Flame Arrow", 450);
		T.setItem(sampleItem11);
		if (T.item().goldValue != 450) {
			System.out.println("The new data item for **Flame Arrow** should now be 450");
		}
		try {
			T.setItem(new Loot("Potato gun", 950));
		...
		T.delete("Durable Spear");

		System.out.println("-----------------------------------------------------------");
		System.out.println(T.toStringByLevel());
		System.out.println("-----------------------------------------------------------");

		T.search("King's Spear");
		System.out.println("Current key of the item cursor is pointing at is: " + T.itemKey());
		T.delete("King's Spear");

		if (T.itemKey().compareTo("Immortality Elixir") != 0) {
			System.out.println("Cursor should point at the item having key as **Immortality Elixir**.");
		}
		...
		T.search("Prince's Dagger");
		System.out.println("Current key of the item cursor is pointing at is: " + T.itemKey());

		T.delete("Prince's Dagger");

		if (T.itemKey().compareTo("Immortality Elixir") != 0) {
			System.out.println("Cursor should point to item having key as **Immortality Elixir**.");
		}
