...
public class PriorityQueue280<I extends Comparable<? super I>> {
	...
	/**
	 * Check if the Priority Queue is full or not
	...
	/**
	 * Checks the number of items in the Priority queue.
	...
	/**
	 * The item is enqueued in the priority order
	 ...
	 * @return reference to the largest priority item in the priority queue
	 * @throws ContainerEmpty280Exception if the queue is empty.
	 */
	public I maxItem() throws ContainerEmpty280Exception {
		if( items.isEmpty()){
			throw new ContainerEmpty280Exception("Can't retrieve a high priority item from an empty queue.");
		}
		return items.item();
	}
	...
	public I minItem() throws ContainerEmpty280Exception{
		if( items.isEmpty()){
			throw new ContainerEmpty280Exception("Can't retrieve the item with lowest priority from an empty queue.");
		}
		ArrayedBinaryTreeIterator280<I> iterator = items.iterator();
		iterator.goFirst();

		I minimum = iterator.item();
		iterator.goForth();

		while(iterator.itemExists()){
			if( iterator.item().compareTo(minimum) < 0 ) {
				minimum = iterator.item();
			}
			iterator.goForth();
		}
		return minimum;
	...
	/**
	 * Delete the item with the lowest priority from the queue
	 */
	public void deleteMin() {
		if( items.isEmpty()){
			throw new ContainerEmpty280Exception("Can't delete an item from an empty queue.");
		}
		I minimum = minItem();
		ArrayedBinaryTreeIterator280<I> iterator = items.iterator();
		iterator.goFirst();
		while(iterator.itemExists()){
			if( iterator.item() == minimum){
	...
	/**
	 * Delete all the items that have the same priority as of the highest priority element
	 * */
	public void deleteAllMax(){
		if( items.isEmpty()){
			throw new ContainerEmpty280Exception("Can't delete the item with highest priority from an empty queue.");
		}
		I maximum = items.item();
		while(items.itemExists() && items.item().compareTo(maximum) == 0){
			items.deleteItem();
		}
	}
	...
	public void deleteMax() throws ContainerEmpty280Exception{
		if( items.isEmpty()){
			throw new ContainerEmpty280Exception("Can't delete the item with the largest priority ");
		}
		items.deleteItem();
	}
