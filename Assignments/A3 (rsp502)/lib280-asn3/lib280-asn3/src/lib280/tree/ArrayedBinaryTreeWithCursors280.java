...
public class ArrayedBinaryTreeWithCursors280<I> extends
	...
	public void insert(I x) throws ContainerFull280Exception, DuplicateItems280Exception {
		if (this.isFull()) {
			throw new ContainerFull280Exception("Tree is already full. Cannot add this item to the tree");
		}
		else {
	...
	public void goForth() throws AfterTheEnd280Exception {
		if(this.after()) throw new Container280Exception("Cursor can't be moved to the after position");
		this.currentNode++;
	}

	@Override
	public void goFirst() throws ContainerEmpty280Exception {
		if(this.isEmpty()) throw new ContainerEmpty280Exception("Cursor can't be moved to the first item");
		this.currentNode=1;
	}
	...
	public void parent() throws InvalidState280Exception {
        if(currentNode <= 1) throw new InvalidState280Exception("Can't move to the parent of the current node ");
		else{
	...
	public void goLeftChild()  throws InvalidState280Exception, ContainerEmpty280Exception {
        if(isEmpty()) {
			throw new ContainerEmpty280Exception("Can't move the cursor to the left child if the tree is Empty");
		}
		int lc = findLeftChild(currentNode);
		if(lc > count){
			throw new InvalidState280Exception("The current node has no left child");
		}
		else{
			currentNode = lc;
		}
	...
	public void goRightChild() throws InvalidState280Exception, ContainerEmpty280Exception {
        if(isEmpty()) {
			throw new ContainerEmpty280Exception("Can't move the cursor to the right child if the tree is Empty");
		}
		int rc = findRightChild(currentNode);
		if(rc > count){
	...
	public void goSibling() throws InvalidState280Exception, ContainerEmpty280Exception {
        if(isEmpty()) {
			throw new ContainerEmpty280Exception("Can't move the cursor in a which tree is Empty");
		}
		if(currentNode%2==0){
			if(currentNode < count ){
				...
				throw new InvalidState280Exception("The node doesn't have a right sibling");
			}
		}
		else{
			if(currentNode<=1){
				throw new InvalidState280Exception("The node doesn't have a left sibling");
			}
			else{
				currentNode--;
			}
	...
	public void root() throws ContainerEmpty280Exception {
        if( isEmpty() ) {
			throw new ContainerEmpty280Exception("The tree is empty and has no root.");
		}
		else{
			currentNode = 1;
		}
